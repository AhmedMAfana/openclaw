...
def test_summary_payload_has_all_required_fields():
    ...
    from openclow.api.serializers.admin_instance import build_status_counts
...
def test_maybe_emit_summary_debounces_to_one_per_minute():
    from openclow.services import instance_service as svc
    ...
    with patch("openclow.services.activity_log.log_event", side_effect=fake_log_event):
...
def test_emit_instance_event_never_raises_on_failure():
    from openclow.services.instance_service import emit_instance_event

    with patch("openclow.services.activity_log.log_event", side_effect=RuntimeError("disk full")):
