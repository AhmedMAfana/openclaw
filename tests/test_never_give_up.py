...
class TestRetrySettings:
    def test_settings_has_retry_config(self):
        src_path = os.path.join("src", "openclow", "settings.py")
...
class TestCancelRaceCondition:
    def test_cancel_handler_checks_user_cancelled(self):
        ...
        assert "openclow:cancel_session" in src, "Cancel handler must check Redis cancel key"
...
class TestCardButtons:
    def test_checklist_reporter_builds_buttons(self):
        src_path = os.path.join("src", "openclow", "services", "checklist_reporter.py")
        with open(src_path) as f:
            src = f.read()
    ...
    def test_discard_allows_failed_status_in_bot_actions(self):
        src_path = os.path.join("src", "openclow", "services", "bot_actions.py")
        with open(src_path) as f:
            src = f.read()
