...
from taghdev.providers.base import LLMProvider, ReviewResult
from taghdev.providers.registry import register_llm
...
def _mcp_git(workspace_path: str) -> dict:
    ...
    return {"command": "python", "args": ["-m", "taghdev.mcp_servers.git_mcp", workspace_path]}
...
def _mcp_docker() -> dict:
    return {"command": "python", "args": ["-m", "taghdev.mcp_servers.docker_mcp"]}
...
def _mcp_instance(instance) -> dict:
    ...
    Takes an ``taghdev.models.Instance`` row. Reads only DB-authoritative
...
def _mcp_workspace(instance) -> dict:
    ...
    return {
        "command": "python",
        "args": [
            "-m", "taghdev.mcp_servers.workspace_mcp",
...
def _coder_env_spec(
    ...
    # Docker mode (default)
    app_container_full = f"taghdev-{project_name}-{app_container}-1"
    tools = common_tools + [
    ...
    env_hint = (
        ...
        f"- Project compose name: taghdev-{project_name}\n"
        f"- App port: {app_port}\n"
    )
    return tools, mcp_servers, env_hint
...
def _mcp_actions() -> dict:
    return {"command": "python", "args": ["-m", "taghdev.mcp_servers.actions_mcp"]}


def _mcp_github() -> dict:
    return {"command": "python", "args": ["-m", "taghdev.mcp_servers.github_mcp"]}
...
class ClaudeProvider(LLMProvider):
    ...
    async def run_coder(
        ...
        app_container_full = f"taghdev-{project_name}-{app_container}-1" if app_container else ""
