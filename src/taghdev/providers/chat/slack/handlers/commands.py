...
from taghdev.providers.chat.slack.handlers.actions import _open_task_modal
from taghdev.providers.chat.slack.middleware import check_auth, is_admin_async
...
def register(app):
    ...
    async def handle_projects(ack, body, client):
        ...
        from taghdev.services.channel_service import get_channel_project
    ...
    async def handle_addproject(ack, body, client):
        ...
        if project_id and project_name:
            try:
                from taghdev.services.tunnel_service import get_tunnel_url, check_tunnel_health
    ...
    async def handle_dev(ack, body, client):
        ...
        if await is_admin_async(user_id):
            ...
            from taghdev.providers.chat.slack.middleware import grant_dev_mode, DEV_SESSION_TTL
    ...
    async def handle_devoff(ack, body, client):
        """Deactivate dev mode."""
        await ack()
        from taghdev.providers.chat.slack.middleware import revoke_dev_mode
