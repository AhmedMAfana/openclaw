...
from taghdev.api.auth import verify_settings_auth
from taghdev.api.schemas.settings import (
    ProjectCreate,
    ProjectResponse,
    ProjectUpdate,
...
from taghdev.providers.registry import available_providers
...
async def update_config(category: str, body: ProviderConfigUpdate):
    ...
    from taghdev.providers.factory import reset
    await reset()
    ...
    if category == "chat":
        if old_type and old_type != provider_type:
            from taghdev.models.task import Task
...
async def _test_database() -> TestResult:
    try:
        from taghdev.models.base import engine
...
async def get_host_settings():
    ...
    from taghdev.services import config_service as _cs
...
async def unlink_channel(provider_type: str, channel_id: str):
    from taghdev.services.channel_service import unset_channel_project
...
async def settings_login(body: dict):
    ...
    from taghdev.api.auth import _get_api_key
    import secrets
