...
from taghdev.utils.docker_path import get_docker_env
...
def _load_functions():
    from taghdev.worker.tasks.orchestrator import execute_task, execute_plan, approve_task, merge_task, reject_task, discard_task
    from taghdev.worker.tasks.onboarding import onboard_project, confirm_project
    from taghdev.worker.tasks.chat_task import chat_response
    from taghdev.worker.tasks.github_tasks import list_github_repos
    from taghdev.worker.tasks.health_task import check_project_health, stop_tunnel_task
    from taghdev.worker.tasks.tunnel_tasks import refresh_dashboard_tunnel, stop_dashboard_tunnel, check_tunnel_health_task
    from taghdev.worker.tasks.bootstrap import bootstrap_project
    from taghdev.worker.tasks.transcribe_task import transcribe_voice
    from taghdev.worker.tasks.logs_task import smart_logs
    from taghdev.worker.tasks.project_lifecycle import docker_up_task, docker_down_task, unlink_project_task, remove_project_task
    from taghdev.worker.tasks.qa_task import run_qa_tests
    from taghdev.worker.tasks.bot_lifecycle import restart_bot_task, get_bot_status_task
    from taghdev.worker.tasks.agent_session import agent_session
    from taghdev.worker.tasks.auth_task import claude_auth_task, claude_auth_check, claude_auth_get_url, claude_auth_login_web
    from taghdev.worker.tasks.instance_tasks import (
        provision_instance,
        teardown_instance,
        rotate_github_token,
    ...
    from taghdev.worker.tasks.maintenance import gc_session_branch
    return [
...
async def on_startup(ctx: dict):
    ...
    from taghdev.services.tunnel_service import start_tunnel
    ...
    # 1. "taghdev-{name}-{taskid}" — old workspace_service with task ID suffix
    # 2. "{name}" (bare) — LLM agent ran `docker compose up` without -p flag
    # Legitimate stacks: "taghdev-{name}" (no extra suffix)
    try:
        ...
        # Build set of legitimate stack names: "taghdev-{project_name}"
        legitimate = {"taghdev"} | {f"taghdev-{name}" for name in known_projects}
    ...
    try:
        ...
        if orphaned:
            for p in orphaned:
                ...
                try:
                    from taghdev.services.project_lock import force_release
    ...
    try:
        ...
        if stuck_tasks:
            from taghdev.worker.tasks.maintenance import _finalize_web_progress_cards
    ...
    try:
        ...
        cursor = 0
        while True:
            cursor, keys = await _r.scan(cursor, match="taghdev:project_lock:*", count=100)
            for key in keys:
    ...
    from taghdev.worker.tasks.maintenance import maintenance_loop
...
async def _restore_project_tunnels():
    ...
    """
    from taghdev.services.tunnel_service import start_tunnel
    from taghdev.services.docker_guard import run_docker
    ...
    try:
        ...
        for p in projects:
            try:
                container = f"taghdev-{p.name}-{p.app_container_name or 'app'}-1"
...
async def _prewarm_whisper():
    ...
    try:
        ...
        from taghdev.worker.tasks.transcribe_task import _get_model
...
async def _tunnel_health_loop():
    ...
    from taghdev.worker.tasks.tunnel_tasks import check_tunnel_health_task
...
async def on_shutdown(ctx: dict):
    ...
    from taghdev.services.audit_service import flush
    await flush()
...
def _load_cron_jobs():
    ...
    """
    from taghdev.services.inactivity_reaper import reaper_cron
    from taghdev.worker.tasks.instance_tasks import (
        tunnel_health_check_cron,
        orphan_compose_sweeper_cron,
    )
