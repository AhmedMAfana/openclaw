...
from taghdev.services.activity_log import log_event
...
async def _find_container_by_service(service: str) -> str | None:
    ...
    Returns the container name (e.g. 'taghdev-bot-1') or None.
...
async def restart_bot_task(ctx: dict, reason: str = "config_change"):
    ...
    if not healthy:
        ...
        try:
            from taghdev.worker.tasks._agent_helper import run_repair_agent
