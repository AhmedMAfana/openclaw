    ...
    try:
        ...
        for line in result.stdout.strip().split("\n"):
            ...
            if not proj or proj in legitimate:
                continue
            # taghdev-{name}-{extra} = task-suffixed orphan
...
async def cleanup_stale_workspaces(max_age_hours: int = 12):
    ...
    for entry in os.listdir(base):
        ...
        try:
            ...
            compose_project = f"taghdev-task-{task_id}"
            subprocess.run(
...
def _docker_daemon_uptime_seconds() -> float | None:
    ...
    try:
        ...
        # Best proxy: check if any taghdev containers were running very recently.
...
async def trim_audit_logs(keep_days: int = 7):
    ...
    from taghdev.models import AuditLog, async_session
    ...
    try:
        ...
        try:
            for project_id, project_name in bootstrapping:
                key = f"taghdev:project_lock:{project_id}"
                ttl = await r.ttl(key)
