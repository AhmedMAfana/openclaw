...
from taghdev.models import Task, TaskLog, WebChatSession, async_session
from taghdev.providers import factory
from taghdev.providers.base import ProviderMismatchError
from taghdev.services.workspace_service import WorkspaceService
...
async def _make_cancel_watcher(chat_id: str) -> "asyncio.Task | None":
    ...
    publishes to taghdev:cancel:{session_id}. This watcher subscribes to
    ...
    async def _watcher():
        ...
        try:
            ...
            await r.delete(f"taghdev:cancel_session:{session_id}")

            # Subscribe to cancel channel — instant notification
            pubsub = r.pubsub()
            await pubsub.subscribe(f"taghdev:cancel:{session_id}")
            try:
                ...
                await pubsub.unsubscribe(f"taghdev:cancel:{session_id}")
                await pubsub.aclose()
                await r.aclose()
...
async def _run_frontend_build(
    ...
    compose_project = f"taghdev-{task.project.name}"
...
def _main_menu_keyboard():
    ...
    from taghdev.providers.actions import terminal_keyboard
...
Compose project: taghdev-{project_name}
...
async def _ensure_project_healthy(task, reporter, task_id_str: str) -> tuple[bool, str | None]:
    ...
    compose_project = f"taghdev-{project.name}"
    ...
    if not is_healthy:
        try:
            from taghdev.services.health_service import find_project_containers
    ...
    if is_healthy and tunnel_url:
        ...
        from taghdev.services.tunnel_service import verify_tunnel_url
    ...
    if is_healthy and not tunnel_url:
        ...
        try:
            from taghdev.services.docker_guard import run_docker as _run_docker
            ...
            from taghdev.services.tunnel_service import start_tunnel, stop_tunnel, verify_tunnel_url, check_tunnel_health
...
async def execute_task(ctx: dict, task_id: str, skip_planning: bool = False):
    ...
    from taghdev.services.project_lock import acquire_project_lock, get_lock_holder
    ...
    if lock is None:
        ...
        try:
            if task.chat_provider_type == "slack":
                from taghdev.providers.chat.slack import blocks
    ...
    try:
        ...
        branch_name = f"taghdev/{task_id_str[:8]}-{branch_slug}"
        ...
        if task.git_mode == "direct_commit":
            ...
            if session_branch:
                ...
                branch_name = f"taghdev/session-{sid_prefix}-{branch_slug}"
                await git_ops.create_branch(workspace.path, branch_name)
                if web_session_id:
                    async with async_session() as db_session:
                        from sqlalchemy import select as sa_select
        ...
        if task.chat_id.startswith("web:"):
            try:
                ...
                if len(parts) >= 3:
                    ...
                    user_cancelled = await r.get(f"taghdev:cancel_session:{parts[2]}") is not None
                    await r.aclose()
            except Exception:
                pass
        await _update_task(task_id_str, status="failed",
...
async def execute_plan(ctx: dict, task_id: str):
    ...
    try:
        ...
        from taghdev.services.pipeline_cache import get_cached_health, set_health_cache
        ...
        if _skip_review:
            ...
            from taghdev.providers.base import ReviewResult
        ...
        if not tunnel_url and task.project.is_dockerized:
            try:
                from taghdev.services.docker_guard import run_docker as _run_docker
                from taghdev.services.tunnel_service import start_tunnel, verify_tunnel_url
                rc, ip_out = await _run_docker(
                    "docker", "inspect",
                    "--format", "{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}",
                    f"taghdev-{task.project.name}-{task.project.app_container_name or 'app'}-1",
                    actor="orchestrator",
                )
        ...
        try:
            from taghdev.services.pipeline_cache import invalidate_health_cache
