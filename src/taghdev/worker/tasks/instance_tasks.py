...
from taghdev.models.instance import FailureCode, Instance, InstanceStatus
...
from taghdev.services.instance_service import InstanceService, emit_instance_event
...
async def _make_workspace_shared_writable(
    *, host_workspace_dir: str | None, slug: str,
    ...
    Two players write to the same files:
      ...
      * **Worker / agent** — runs as ``taghdev`` (uid 1000) inside the
    ...
    # taghdev (uid 1000). chmod 0777 doesn't require root — only
...
async def _capture_projctl_diagnostics(*, slug: str) -> None:
    ...
    look at /var/log/taghdev/failed-instances/<slug>-<ts>/ and see
    *why* the step failed.
...
async def _publish_progress_step(
    *,
    ...
    try:
        import json as _pj
        from taghdev.models.web_chat import WebChatMessage, WebChatSession
