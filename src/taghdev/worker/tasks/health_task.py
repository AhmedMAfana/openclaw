...
from taghdev.services.health_service import (
    HealthReport, run_full_health_check, find_project_containers,
)
from taghdev.services.tunnel_service import stop_tunnel as _stop_tunnel
...
from taghdev.services.base_reporter import edit_message as _notify  # noqa: E402
...
async def _sync_app_url(workspace: str, tunnel_url: str, compose_project: str, project):
    ...
    from taghdev.services.tunnel_service import sync_project_tunnel
...
async def _notify_with_buttons(chat, chat_id, message_id, text, project_id, tunnel_url=None):
    ...
    from taghdev.providers.actions import open_app_btn
    extra = []
...
async def _run_repair_loop(
    ...
    from taghdev.worker.tasks.bootstrap import _run_master_agent
    ...
    from taghdev.services.docker_guard import _detect_host_workspace_path
    from taghdev.settings import settings as _settings
    ...
    from taghdev.services.config_service import get_config as _gc, set_config as _sc
    ...
    from taghdev.services.tunnel_service import refresh_tunnel
    from taghdev.worker.tasks.bootstrap import _get_tunnel_target
    from taghdev.services.port_allocator import get_app_port as _get_app_port
...
async def check_project_health(ctx: dict, project_id: int, chat_id: str, message_id: str, chat_provider_type: str = "telegram"):
    ...
    try:
        ...
        from taghdev.settings import settings
        from taghdev.services.checklist_reporter import ChecklistReporter
        from taghdev.services.tunnel_service import get_tunnel_url, check_tunnel_health, ensure_tunnel
        from taghdev.worker.tasks.bootstrap import _get_tunnel_target
        from taghdev.services.port_allocator import get_app_port
        from taghdev.providers.actions import ActionButton, ActionKeyboard, ActionRow, project_nav_keyboard, open_app_btn
        import aiohttp
...
async def stop_tunnel_task(ctx: dict, project_id: int, chat_id: str, message_id: str, chat_provider_type: str = "telegram"):
    ...
    try:
        ...
        from taghdev.providers.actions import ActionButton, project_nav_keyboard, nav_keyboard
        ...
        from taghdev.providers.actions import project_nav_keyboard
...
async def _check_project_health_host(project, chat, chat_id: str, message_id: str):
    ...
    from taghdev.services.tunnel_service import get_tunnel_url, ensure_tunnel
    from taghdev.providers.actions import (
    ...
    try:
        ...
        if not process_running and project.project_dir and project.start_command:
            ...
            log_path = f"{project.project_dir}/.taghdev-start.log"
            rc, _ = await run_host(
