...
async def _run(*args: str, cwd: str = None, timeout: int = 300,
    ...
    """
    from taghdev.services.audit_service import log_action
...
from taghdev.services.checklist_reporter import ChecklistReporter  # noqa: E402
...
async def _preflight(project, workspace: str, compose: str, compose_project: str):
    ...
    """
    from taghdev.services.docker_guard import run_docker_compose, _detect_host_workspace_path
    from taghdev.services.port_allocator import get_port_env_vars
    ...
    # 3. Kill orphan stacks with task ID suffixes (taghdev-{name}-{taskid})
    try:
        proc = await asyncio.create_subprocess_exec(
            "docker", "ps", "-a", "--filter", f"name=taghdev-{project.name}-",
            "--format", "{{.Names}}",
            stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE,
            env=_denv,
        )
        ...
        for name in stdout.decode().strip().split("\n"):
            ...
            if len(parts) == 2:
                ...
                if stack != compose_project and stack.startswith(f"taghdev-{project.name}"):
                    orphans.add(stack)
    ...
    # 7. Stop old tunnel (recreated after app verified)
    try:
        from taghdev.services.tunnel_service import stop_tunnel
...
async def _get_compose_containers(
    ...
    for line in ps_output.strip().split("\n"):
        ...
        try:
            ...
            # Extract service name: "taghdev-tagh-test-nginx-1" → "nginx"
...
async def _run_master_agent(
    ...
    async def _web_cancelled() -> bool:
        ...
        try:
            ...
            val = await r.get(f"taghdev:cancel_session:{_cancel_session_id}")
            await r.aclose()
...
async def bootstrap_project(ctx: dict, project_id: int, chat_id: str, message_id: str, chat_provider_type: str = "telegram"):
    ...
    if chat_id.startswith("web:"):
        try:
            ...
            if len(_parts) == 3:
                ...
                await _rc.delete(f"taghdev:cancel_session:{_parts[2]}")
                await _rc.aclose()
    ...
    from taghdev.services.port_allocator import get_app_port
    ...
    try:
        ...
        async def _bail(reason: str):
            ...
            from taghdev.services.docker_guard import run_docker_compose
            try:
                await run_docker_compose(
                    "down", "--remove-orphans",
        ...
        try:
            ...
            if url:
                ...
                from taghdev.services.tunnel_service import verify_tunnel_url, stop_tunnel
...
STEP 5 — Health check
  ...
  If failed: host_tail_log (check .taghdev-start.log and any app log), diagnose, fix,
...
async def _bootstrap_project_host(
    ...
    try:
        from claude_agent_sdk import query, ClaudeAgentOptions
        from claude_agent_sdk.types import AssistantMessage, TextBlock, ToolUseBlock
        from taghdev.providers.llm.claude import _mcp_host
        ...
        if configured_url and not tunnel_enabled:
            ...
            try:
                from taghdev.services.tunnel_service import start_tunnel, stop_tunnel, verify_tunnel_url
        ...
        from taghdev.providers.actions import ActionButton, ActionKeyboard, ActionRow, open_app_btns
        rows = [
